    ...
    function createCircuitBreaker(withVersion) {
      return {
        ...
        async parse(context) {
          ...
          const version = withVersion ? await exports.versionFlag.parse(context) : void 0;
...
var appendFile = util.promisify(require("fs").appendFile);
var cp = util.promisify(require("fs").cp);
var exists = util.promisify(require("fs").exists);
var mkdir = util.promisify(require("fs").mkdir);
...
var installDependencies = () => __async(void 0, null, function* () {
  yield Log.spinner(
    `Installing dependencies...`,
    (spinner) => __async(void 0, null, function* () {
      for (const dep of deps) {
        spinner.text = `Installing dependencies...${dep}`;
        yield exec(`yarn add ${dep}`);
      }
    }),
    "Installing dependencies"
  );
});
var createTsConfig = (projectRoot) => __async(void 0, null, function* () {
  const tsConfigLocation = (0, import_path.resolve)(projectRoot, "tsconfig.json");
  const tsConfig = {
    compilerOptions: {
      target: "es6",
...
var createPackageJson = (projectRoot) => __async(void 0, null, function* () {
  const packageJsonLocation = (0, import_path.resolve)(projectRoot, "package.json");
  const packageJson = JSON.parse(yield readFile(packageJsonLocation, "utf-8"));
...
var initYarn = () => __async(void 0, null, function* () {
  yield exec("yarn set version berry");
  yield exec("yarn init");
});
var createProjectDirectory = (projectRoot) => __async(void 0, null, function* () {
  try {
    yield Log.spinner("Creating project directory", () => __async(void 0, null, function* () {
      if (yield exists(projectRoot)) {
        throw new Error("Project directory is not empty");
      } else {
        yield mkdir(projectRoot);
      }
    }));
  } catch (e) {
    if (e.message === "Project directory is not empty") {
...
var copyTemplateFiles = (projectRoot) => __async(void 0, null, function* () {
  const source = (0, import_path.resolve)(__dirname, "./../template");
  yield Log.spinner(`Copying from '${source}' to ${projectRoot}`, () => __async(void 0, null, function* () {
    yield cp(source, projectRoot, { recursive: true });
  }));
});
var updateGitIgnore = (projectRoot) => __async(void 0, null, function* () {
  const gitIgnoreLines = ["\n", "dist/", "keys/"];
  yield appendFile(
    (0, import_path.resolve)(projectRoot, ".gitignore"),
    gitIgnoreLines.join("\n"),
    "utf-8"
  );
});
var createApp = (name) => {
  const createAppTasks = () => __async(void 0, null, function* () {
    const projectRoot = (0, import_path.resolve)((0, import_path.relative)(process8.cwd(), name));
    yield createProjectDirectory(projectRoot);
    process8.chdir(projectRoot);
    yield Log.spinner("Initializing yarn project", () => initYarn());
    yield copyTemplateFiles(projectRoot);
    yield installDependencies();
    yield Log.spinner("Setting up project files", () => __async(void 0, null, function* () {
      yield createPackageJson(projectRoot);
...
var app = (0, import_cmd_ts.command)({
  name: "create-crx-app",
  args: {
    name: (0, import_cmd_ts.positional)({ type: import_cmd_ts.string, displayName: "Project name" })
  },
  handler: ({ name }) => {
    createApp(name);
  }
